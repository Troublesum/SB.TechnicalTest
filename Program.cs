...
namespace SB.TechnicalTest
{
    class Program
    {
        ...
        static int Attempt2()
        {
            var currentFloor = Building.NumberFloors;
            var addFloors = currentFloor;
            var increment = currentFloor;
            var lastResult = true;

            while (addFloors != 0)
            {
                var goHigher = Building.DropMarble(currentFloor) && currentFloor + addFloors / increment <= Building.NumberFloors;

                increment = setIncrement(lastResult, goHigher, increment);

                addFloors = (goHigher) ? increment : -increment;

                lastResult = goHigher;

                currentFloor += addFloors;
            }

            return currentFloor;
        }

        private static int setIncrement(bool lastResult, bool newResult, int currentIncrement)
        {
            if(lastResult != newResult)
                return currentIncrement / 2;
